...
class WebDashboard {
  ...
  private async manualEventSync(req: any, res: any) {
    try {
      logger.info('Starting manual full sync triggered by user');
      ...
      logger.error(error, 'Manual full sync failed');
  ...
  private async startSync() {
    ...
    logger.info({ fullSyncInterval }, 'Full sync service started (no event-based sync)');
  }
  ...
  private stopSync() {
    ...
    logger.info('Full sync service stopped');
  }
