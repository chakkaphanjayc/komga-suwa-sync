...
export class KomgaClient {
    ...
    try {
      while (true) {
        ...
        if (data.content && data.content.length > 0) {
          ...
          logger.info({ page, count: data.content.length, total: allSeries.length }, 'Fetched Komga series page');
        }
      ...
      logger.info({ total: allSeries.length }, 'Fetched all Komga series');
      ...
      logger.error(error, 'Failed to fetch Komga series');
  ...
  async getBooks(seriesId: string) {
    const allBooks: any[] = [];
    let page = 0;
  ...
  async getReadProgress(bookId: string) {
    try {
      ...
      if (error.response?.status === 404) {
        ...
        logger.info({ bookId }, 'No read progress found for book, using default');
        return {
          page: 0,
          completed: false,
          readDate: null
        ...
        try {
          ...
          logger.warn({ bookId, error: altError.message }, 'Alternative read progress method failed, using default');
          return {
            page: 0,
            completed: false,
            readDate: null
        ...
        logger.warn({ bookId, error: error.message }, 'Read progress request failed, using default');
        return {
          page: 0,
          completed: false,
          readDate: null
  ...
  async patchReadProgress(bookId: string, page: number, completed: boolean) {
    if (process.env.SYNC_DRY_RUN === 'true') {
      logger.info({ bookId, page, completed }, 'DRY RUN: Would patch Komga read progress');
      return;
    }
    ...
    try {
      ...
      logger.error({ bookId, page, completed, error: error.message }, 'Failed to patch Komga read progress');
